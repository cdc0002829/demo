...
public class MeetingListAdapter extends RecyclerView.Adapter<MeetingListViewHolder> implements View.OnClickListener {
    ...
    private ConsultingRoomActivity myConsultingRoomActivity;
    ...
    public MeetingListAdapter(ConsultingRoomActivity myConsultingRoomActivity, List<MeetingItem> mDatas, Context context) {
        this.myConsultingRoomActivity = myConsultingRoomActivity;
        this.mDatas = mDatas;
    ...
    public void onBindViewHolder(MeetingListViewHolder holder, int position) {
        ...
        if (mDatas.get(position).meetingType == 1) {
            if (mDatas.get(position).creatorId.equals(AccountManager.getInstance(mContext).getNube())) {
                holder.meetingTopic.setText(R.string.my_consult_meeting_txt);
            } else {
                if (mDatas.get(position).topic.equals("")||holder.meetingTopic==null) {
                    holder.meetingTopic.setText(mDatas.get(position).creatorName + "会诊室");
                } else {
