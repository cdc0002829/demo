...
public class ReserveSuccessActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initData() {
        if (meetinfo != null) {
            ...
            if (TextUtils.isEmpty(meetinfo.getBookName())) {
                tvReserveMeetingPerson.setText("未命名");
            } else {
    ...
    private  boolean isWeixinAvilible() {
        ...
        CustomToast.show(ReserveSuccessActivity.this, "目前您的微信版本过低或未安装微信，需要安装微信才能使用", CustomToast.LENGTH_LONG);
        return false;
    }
    ...
    private void joinMeeting() {
        ...
        switch (res){
            case  0:
                showLoadingView("正在加入会诊");
                isJoinMeeting=true;
                break;
            case MedicalMeetingManage.NETWORKINVISIBLE:
                    CustomToast.show(ReserveSuccessActivity.this, "网络不给力，请检查网络！",
                        CustomToast.LENGTH_SHORT);
                    break;
            default:
                CustomToast.show(ReserveSuccessActivity.this, "加入会议失败!",
                    CustomToast.LENGTH_SHORT);
        }
    }
