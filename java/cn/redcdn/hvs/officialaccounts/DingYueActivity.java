...
public class DingYueActivity extends BaseActivity {
    ...
    private void initView() {
        imageOne = (RoundImageView) findViewById(R.id.imageone_iv);
            ...
            case R.id.dingyuejia_btn:
                final MDSAppFocusoffacc mdsAppFocusoffacc = new MDSAppFocusoffacc() {
                    ...
                    protected void onSuccess(JSONObject responseContent) {
                        ...
                        removeLoadingView();
                        CustomToast.show(DingYueActivity.this, "订阅成功", 7000);
                    ...
                    protected void onFail(int statusCode, String statusInfo) {
                        ...
                        removeLoadingView();
                        CustomToast.show(DingYueActivity.this, "订阅失败，请重新订阅", 7000);
                        dingyueButton.setVisibility(View.VISIBLE);
                        subscribeState = false;
                    }
                };
                DingYueActivity.this.showLoadingView("订阅中", new DialogInterface.OnCancelListener() {

                    @Override
                    public void onCancel(DialogInterface dialog) {
                        dialog.dismiss();
                        mdsAppFocusoffacc.cancel();
                        CustomToast.show(getApplicationContext(), "取消订阅", Toast.LENGTH_SHORT);
                    }
                });
                mdsAppFocusoffacc.appFocusoffacc(AccountManager.getInstance(getApplicationContext()).getToken(), officialAccountId, 2);
        ...
        if (subscribeState == false) {
            ...
            final MDSAppFocusoffacc mdsAppFocusoffacc = new MDSAppFocusoffacc() {
                ...
                protected void onSuccess(JSONObject responseContent) {
                    super.onSuccess(responseContent);
                    removeLoadingView();
                    CustomToast.show(DingYueActivity.this, "订阅已取消", 7000);
                    dingyueButton.setVisibility(View.VISIBLE);
                    subscribeState = false;
                }
                ...
                protected void onFail(int statusCode, String statusInfo) {
                    super.onFail(statusCode, statusInfo);
                    removeLoadingView();
                    CustomToast.show(DingYueActivity.this, "取消订阅失败，请重新取消订阅", 7000);
                    dingyueButton.setVisibility(View.GONE);
                }
            };
            DingYueActivity.this.showLoadingView("取消订阅中", new DialogInterface.OnCancelListener() {

                @Override
                public void onCancel(DialogInterface dialog) {
                    dialog.dismiss();
                    mdsAppFocusoffacc.cancel();
                    CustomToast.show(getApplicationContext(), "停止取消订阅", Toast.LENGTH_SHORT);
                }
            });
            mdsAppFocusoffacc.appFocusoffacc(AccountManager.getInstance(getApplicationContext()).getToken(), officialAccountId, 1);
        }
    }
    ...
    private void showDialog() {
        ...
        if (subscribeState == true) {
            choosePhoto.setText("不再订阅");
            choosePhoto.setTextColor(Color.RED);
        } else {
            choosePhoto.setText("+订阅");
        }
        //将布局设置给Dialog
    ...
    private void findOfficialDate() {
        final MDSAppGetOffAccInfo offAccInfo = new MDSAppGetOffAccInfo() {
            ...
            protected void onFail(int statusCode, String statusInfo) {
                ...
                CustomToast.show(getApplicationContext(), "加载数据失败",
                        Toast.LENGTH_SHORT);
                DingYueActivity.this.finish();
            }
        };

        offAccInfo.appGetOffAccInfo(AccountManager.getInstance(MedicalApplication.context).getToken(), officialAccountId);

        DingYueActivity.this.showLoadingView("加载中", new DialogInterface.OnCancelListener() {
