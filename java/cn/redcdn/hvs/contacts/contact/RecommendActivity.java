...
import cn.redcdn.hvs.contacts.contact.RecommendListViewAdapter.buttonClick;
...
public class RecommendActivity extends BaseActivity {
  ...
  private void initAdapter() {
    recommendAdapter = new RecommendListViewAdapter(this, newRecommendContact,
        new bc());
  ...
  public class bc implements buttonClick {
    ...
    public void itemClicked(View v, int position) {
      ...
      if (newRecommendContact.get(position).getBeAdded() == 1) {
        return;
      } else {
        CustomLog.d(
            TAG,
            "v=" + v + "position=" + position
                + "newRecommendContact.get(position)="
                + newRecommendContact.get(position));
        ((TextView) v).setText("已添加");
        ((TextView) v).setTextColor(0xffc8cacc);
        ((TextView) v).setClickable(false);
        CustomLog.d(TAG, newRecommendContact.get(position).toString());
        ContactManager.getInstance(RecommendActivity.this).
        addContact(newRecommendContact.get(position), 
    			new ContactCallback(){
        	@Override
    		public void onFinished(ResponseEntry result) {
    			...
    			if (result.status >= 0) {
    				 newRecommendContact.get(pos).setBeAdded(1);
  ...
  public String[] sortIndex(List<Contact> mContacts) {
    ...
    for (Contact mContact : mContacts) {
      ...
      // if (mContact.getNickname() != null &&
      // !mContact.getNickname().isEmpty()) {
