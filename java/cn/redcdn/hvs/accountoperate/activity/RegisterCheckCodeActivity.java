...
public class RegisterCheckCodeActivity extends BaseActivity {
	...
	private void reSendActivateCode() {
		RegisterCheckCodeActivity.this.showLoadingView("获取中",
		...
		rac = new ReSendActivateCode() {
			...
			protected void onFail(int statusCode, String statusInfo) {
				...
				if (statusCode == -401) {
					CustomToast.show(RegisterCheckCodeActivity.this,
							"账号未激活成功", Toast.LENGTH_LONG);
					return;
				}
				if (statusCode == -404) {
					CustomToast.show(RegisterCheckCodeActivity.this,
							"激活码不正确，请重新获取激活码", Toast.LENGTH_LONG);
					return;
				} else {
					CustomToast.show(RegisterCheckCodeActivity.this, "激活码获取失败="
	...
	private void accountActivate() {
		...
		if (checkCode != null && !checkCode.equalsIgnoreCase("")) {
			if (checkCode.length() < 6)
				CustomToast.show(RegisterCheckCodeActivity.this, "请输入六位数字验证码",
						Toast.LENGTH_LONG);
			else {
				RegisterCheckCodeActivity.this.showLoadingView("验证中",
						new OnCancelListener() {
				...
				aa = new ActivateAccount() {
					...
					protected void onSuccess(UserInfo responseContent) {
						...
						RegisterCheckCodeActivity.this.showLoadingView("自动登录中",
								new OnCancelListener() {
								...
								.registerLoginCallback(new AccountManager.LoginListener() {
									...
									public void onLoginFailed(int errorCode,
										...
										if (errorCode==MDSErrorCode.MDS_NUBE_NOT_EXIST){
											...
											CustomToast.show(RegisterCheckCodeActivity.this,"自动登录失败,请重新登录",CustomToast.LENGTH_LONG);
					...
					protected void onFail(int statusCode, String statusInfo) {
						...
						if (statusCode == -404) {
							...
							CustomToast.show(RegisterCheckCodeActivity.this,
									"账号未激活成功=" + statusCode, Toast.LENGTH_LONG);
						}
			...
			CustomToast.show(RegisterCheckCodeActivity.this, "验证码不能为空",
					Toast.LENGTH_LONG);
		}
	}
