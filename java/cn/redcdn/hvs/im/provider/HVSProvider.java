    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            case QUERY_UNREAD_NOTICES_COUNT:
                sql = "select t1.id from t_notices t1  left join t_threads t2 on t1.threadsId = t2.id " +
                        "where t1.isNews  = 1 and t2.reserverStr1 = '0'";
                cursor = db.rawQuery(sql, null);
                break;
//                CustomLog.d(TAG, "查询所有未读消息" + cursor.getCount());
            ...
            case CHECK_CONTACT_RECORD:
                ...
                sql = "select contactId contactId from t_callrecords "
                    ...
                    + nubeNumbers + "'";
                CustomLog.d(TAG, "查询联系人是否有通话记录" + sql);
                cursor = db.rawQuery(sql, null);
                break;
            ...
            case QUERY_NUBE_FRIEND_RECORD:
                sql = "select t.nubeNumber as nubeNumber,n.id as id from t_nubefriend t inner join "
                    ...
                    + "where t.isDeleted=0";
                CustomLog.d(TAG, "查询视频表与新朋友表具有相同nube号的记录" + sql);
                cursor = db.rawQuery(sql, null);
                break;
            ...
            case UPLOAD_GET_COUNT:
                ...
                sql = "select count(contactId) scount,isMutualTrust isMutualTrust from "
                    ...
                    + " where syncStat=0";
                CustomLog.d(TAG, "得到需要上传的数据条数" + sql);
                cursor = db.rawQuery(sql, null);
                break;
            case QUERY_NUBE_FRIENDS_LIST:
                ...
                CustomLog.d(TAG, "获取需要同步的佰酷好友数据" + sql);
                cursor = db.rawQuery(sql, null);
                break;
            case QUERY_EMPTY_LINKMAN_ITEM:
                ...
                CustomLog.d(TAG, "查询手机号码为空的数据" + sql);
                cursor = db.rawQuery(sql, null);
                break;
            ...
            case NET_PHONE_NOTICE_LIST:
                ...
                CustomLog.d(TAG, "NET_PHONE_NOTICE_LIST:" + cnt + "   NET_PHONE_NOTICE_LIST");
                break;
            case QUERY_ALL_NOTICES: {
                ...
                sql =
                    " select n.*, tm.nickName,tm.phoneNum,tm.nubeNumber,tm.showName,tm.headUrl,tm.gender," +
                        ...
                        "tf.name from t_notices n left join t_nubefriend tf on n.sender=tf.nubeNumber " +
                        "and tf.isDeleted=0 " +
                        ...
                        " asc ";
                cursor = db.rawQuery(sql, null);
                // qb.appendWhere(NoticesTable.NOTICE_COLUMN_ID + "="
                // + uri.getLastPathSegment());
                // qb.setTables(NoticesTable.TABLENAME);
            ...
            case QUERY_COND_NOTICES:
                CustomLog.d(TAG, "provider NET_PHONE_NOTICE_LIST");
                ...
                CustomLog.d(TAG, "NET_PHONE_NOTICE_LIST:" + i + "   NET_PHONE_NOTICE_LIST");
                break;
            case QUERY_PAGE_NOTICES:
                ...
                sql =
                    " select * from (select n.*, tm.nickName,tm.phoneNum,tm.nubeNumber,tm.showName,tm.headUrl," +
                        ...
                        + " where n.threadsId = '"
                        + convstId + "'";
                //             sql = "select * from (select * from " + NoticesTable.TABLENAME
                //             + " where " + NoticesTable.NOTICE_COLUMN_THREADSID + " = '"
                //             + convstId + "'";
                ...
                CustomLog.d(TAG, "分页查询动态消息:" + sql);
                cursor = db.rawQuery(sql, null);
                int j = cursor.getCount();
                CustomLog.d(TAG, "NET_PHONE_NOTICE_LIST:" + j + "   QUERY_PAGE_NOTICES");
                break;
            ...
            case NETPHONE_THREADS_GETALL:
                ...
                CustomLog.d(TAG, "provider 查询会话消息");
                sql = "select th.id as threadsId,f.name,n2.nickName,n2.phoneNum,n2.nubeNumber"
                    ...
                    + ",f.headUrl from t_notices n1 left join (select * from t_nubefriend "
                    + "where isDeleted=0) f "
                    +
                    ...
                    + "tg on th.id = tg.gid left join (select * from t_nubefriend "
                    +
                    "where isDeleted=0) f on th.recipientIds = f.nubeNumber order by th.top desc,th.lastTime desc";
                cursor = db.rawQuery(sql, null);
                break;
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case NET_PHONE_NOTICE_LIST:
                CustomLog.d(TAG, "provider insert notices data item");
...
//                }
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        // Cursor cursor;
        // Uri _uri = null;
        // String time = DateUtil.getDBOperateTime();
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case NET_PHONE_NOTICE_ITEM:
                ...
                break;
            case NET_PHONE_NOTICE_LIST:
                CustomLog.d(TAG, "provider update notices data list");
