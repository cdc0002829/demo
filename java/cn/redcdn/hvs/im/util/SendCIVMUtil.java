...
import cn.redcdn.hvs.im.activity.RecordingVideoActivity;
...
public class SendCIVMUtil {
    ...
    public static void sendPic(final Activity activity) {
        ...
        //         activity.getLocalClassName(), 300);
        // dialog.setCanceledOnTouchOutside(true);
        // dialog.addSpecialView(R.layout.me_modify_user_photo);
        // TextView title = (TextView) dialog.getContentView().findViewById(
        //         R.id.title);
        // title.setText(R.string.chat_pic_title);
        // TextView titleView = (TextView) dialog.getContentView().findViewById(
        //         R.id.tv_title);
        // titleView.setVisibility(View.GONE);
        // LinearLayout operation_layout = (LinearLayout) dialog.getContentView()
        //         .findViewById(R.id.operation_layout);
        // operation_layout.setVisibility(View.GONE);
        // View view = (View) dialog.getContentView().findViewById(R.id.line2);
        // view.setVisibility(View.GONE);
        // View view2 = (View) dialog.getContentView().findViewById(R.id.line1);
    ...
    //        dialog.showDialog();
    //        Button btn1 = (Button) dialog.getContentView().findViewById(
    //            R.id.button1);
    ...
    public static boolean onSendVideoFromNativeBack(final Activity activity,
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                // 视频每次只能发一个
                String videoPath = selectedVideoList.get(0);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                String filePath = data
                    .getStringExtra(RecordedVideoActivity.KEY_VIDEO_FILE_PATH);
                if (!TextUtils.isEmpty(filePath)) {
                    IMCommonUtil.scanFileAsync(activity, filePath);
                }
                // 视频时长
                int duration = data.getIntExtra(
                    RecordedVideoActivity.KEY_VIDEO_FILE_DURATION, 0) * 1000;
    ...
    public static boolean conveneMeeting(final Activity activity, String tag, int conversationType, final String targetId, final String selfNube) {
        ...
        operationCode = 0;
        if (!checkNetWork(activity)) {
            showToast(activity, "网络不给力，请检查网络！");
            return false;
        }
        mActivity = activity;
        ...
        int reslut = meetingManager.createMeeting(TAG, nubeList,
            new MedicalMeetingManage.OnCreateMeeetingListener() {
                @Override
                public void onCreateMeeting(int valueCode, MeetingInfo meetingInfo) {
                    ...
                    if (valueCode != 0) {
                        ...
                        removeLoadingView();
                        CustomToast.show(activity, "创建会诊失败，请重试", 1);
                        return;
                    }
                    ...
                    if (joinResult == 0) {
                        ...
                        CustomLog.d(TAG, "加入会诊失败");
                        CustomToast.show(activity, "加入会诊失败，请重试", 1);
                        removeLoadingView();
                    }
                }
            });
