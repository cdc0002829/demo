...
public class NoticesDao {
    ...
    private static final String TAG = "NoticesDao";
    ...
    private ThreadsDao threadsDao = null;

    public NoticesDao(Context context) {
        this.mcontext = context;
        this.threadsDao = new ThreadsDao(mcontext);
    }

    /**
     * @author: zhaguitao
     * @Title: queryConvstNoticesCursor
    ...
    public Cursor queryConvstNoticesCursor(String convstId) throws Exception {
        ...
        Uri uri = Uri.withAppendedPath(ProviderConstant.MEDICAL_URI,
                "QUERY_ALL_NOTICES" + "/" + convstId);
        try {
            return mcontext.getContentResolver().query(uri, null, null, null, null);
//            return mcontext.getContentResolver().query(
//                    ProviderConstant.NETPHONE_NOTICE_URI, select_columns,
//                    NoticesTable.NOTICE_COLUMN_THREADSID + " = ? ",
//                    new String[] { convstId },
//                    NoticesTable.NOTICE_COLUMN_RECEIVEDTIME + " ASC");
    ...
    public Cursor queryNotices(String convstId, long recvTimeBegin) throws Exception {
        ...
        Uri uri = Uri.withAppendedPath(ProviderConstant.MEDICAL_URI,
                "QUERY_COND_NOTICES" + "/" + convstId + "/" + recvTimeBegin);
        try {
            return mcontext.getContentResolver().query(uri, null, null, null, null);
//            return mcontext.getContentResolver().query(
//                    ProviderConstant.NETPHONE_NOTICE_URI, select_columns,
...
//                    NoticesTable.NOTICE_COLUMN_RECEIVEDTIME + " ASC");
    ...
    public Cursor queryPageNotices(String convstId, long recvTimeBegin, int pageCnt)
        ...
        Uri uri = Uri.withAppendedPath(ProviderConstant.MEDICAL_URI,
                "QUERY_PAGE_NOTICES" + "/" + convstId + "/" + recvTimeBegin + "/" + pageCnt);
        try {
            return mcontext.getContentResolver().query(uri, null, null, null, null);
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
    ...
    private Cursor getAllPAVInConvst(String threadid, int type)
        ...
        try {
            return mcontext.getContentResolver().query(
                    ProviderConstant.NETPHONE_NOTICE_URI,
                    select_columns,
                    NoticesTable.NOTICE_COLUMN_THREADSID + " = ? AND "
    ...
    public Cursor queryNoRunningSendNotices() {
        ...
        try {
            cursor = mcontext.getContentResolver().query(
                    ProviderConstant.NETPHONE_NOTICE_URI,
                    select_columns,
                    "(" + NoticesTable.NOTICE_COLUMN_STATUS + "=? or "
                            + NoticesTable.NOTICE_COLUMN_STATUS
                            + "=?) AND CAST(" + NoticesTable.NOTICE_COLUMN_TYPE
                            + " AS INT) < ?",
        ...
        if (value != null) {
            try {
                url = mcontext.getContentResolver().insert(
                        ProviderConstant.NETPHONE_NOTICE_URI, value);
            } catch (Exception e) {
                e.printStackTrace();
                CustomLog.e(TAG,"Exception" + e.toString());
            }
    ...
    public int updateNotice(String uuid, String body, int status) {
        ...
        try {
            count = mcontext.getContentResolver()
                    .update(ProviderConstant.NETPHONE_NOTICE_URI, value,
                            NoticesTable.NOTICE_COLUMN_ID + "=?",
                            new String[] { uuid });
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
    ...
    public int updateAudioIsRead(String uuid, boolean read) {
        ...
        try {
            ...
            values.put(NoticesTable.NOTICE_COLUMN_ISREAD, read ? 0 : 1);
            count = mcontext.getContentResolver().update(
                    ProviderConstant.NETPHONE_NOTICE_URI, values,
                    NoticesTable.NOTICE_COLUMN_ID + " = ? ",
                    new String[] { uuid });
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
    ...
    public synchronized Cursor getUnreadNotice(String threadsid)
        ...
        try {
            cursor = mcontext.getContentResolver().query(
                    ...
                    NoticesTable.NOTICE_COLUMN_ISNEW + " = ? AND "
                            + NoticesTable.NOTICE_COLUMN_THREADSID + " = ? ",
    ...
    public  int updateNewStatus(String uuid){
        ...
        try {
            ...
            count = mcontext.getContentResolver().update(
                    ProviderConstant.NETPHONE_NOTICE_URI,
                    values,
                    NoticesTable.NOTICE_COLUMN_ID + " = ? AND "
                            + NoticesTable.NOTICE_COLUMN_ISNEW + " = ? ",
                    new String[] { uuid, "1" });
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
    ...
    /**
     * 获得所有未读新消息的数目（isNew字段） 主要用在主页面底部的红圈显示
     *
     * @return 返回新消息的数目
     */
...
//                    NoticesTable.NOTICE_COLUMN_RECEIVEDTIME + " ASC ");
    ...
    public int deleteNotice(String uuid) {
        ...
        try {
            count = mcontext.getContentResolver()
                    .delete(ProviderConstant.NETPHONE_NOTICE_URI,
                            NoticesTable.NOTICE_COLUMN_ID + "=?",
                            new String[] { uuid });
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
    ...
    public int deleteNotices(List<String> uuids){
        ...
        try {
            count = mcontext.getContentResolver()
                    .delete(ProviderConstant.NETPHONE_NOTICE_URI,
                            NoticesTable.NOTICE_COLUMN_ID + " IN ( "+StringUtil.list2DBINString(uuids)+" )",
                            new String[] {});
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
        }
     ...
     * @param threadid
     *            该记录的threadid
     * @param preRevieverTime
     *            前一条记录的接收时间 ； 如果是空串，则不更新会话表的lasttime字段
     * @return
     */
    public int deleteLastNotice(String uuid, String threadid,
        ...
        if (count > 0 && !TextUtils.isEmpty(threadid) && (preRevieverTime > 0)) {
            ...
            threadsDao.updateLastTime(threadid, preRevieverTime);
        }
        return count;
    }
    ...
    private long getLastItemRecTime(String threadid) {
        ...
        try {
            cursor = mcontext.getContentResolver().query(
                    ...
                    NoticesTable.NOTICE_COLUMN_THREADSID + " = ?",
                    new String[] { threadid },
                    NoticesTable.NOTICE_COLUMN_RECEIVEDTIME + " DESC");
            if (cursor != null && cursor.moveToFirst()) {
                rectime = cursor
                        .getLong(cursor
                                .getColumnIndex(NoticesTable.NOTICE_COLUMN_RECEIVEDTIME));
            }
        } catch (Exception e) {
            CustomLog.e(TAG,"Exception" + e.toString());
        } finally {
    ...
    /**
     * 变更会话的lasttime字段； 先检索到该会话下最近一条消息，拿到其接收时间rectime；
     * 再将rectime设置到会话表的lasttime字段
     *
     * @param threadid
     *            会话的id
     * @return booelan
     */
    public boolean updateThreadTime(String threadid) {
        long lasttime = getLastItemRecTime(threadid);
        if (lasttime > 0) {
            return threadsDao.updateLastTime(threadid, lasttime);
        }
        return false;
    }
    ...
    public int deleteNoticesByDay(String dbDay) {
        ...
        try {
            count = mcontext.getContentResolver().delete(
                    ProviderConstant.NETPHONE_NOTICE_URI,
                    "substr(" + NoticesTable.NOTICE_COLUMN_RECEIVEDTIME
    ...
    public String createSendFileNotice(String sender, String receiver,
        ...
        if (TextUtils.isEmpty(threadid)) {
            ...
            if (TextUtils.isEmpty(covstid)) {
                CustomLog.d(TAG,"createSendFileNotice createThread id==null");
                return "";
            }
            threadid = covstid;
            bean.setThreadsId(covstid);
        } else {
            if (!threadsDao.isExistThread(threadid)) {
                threadsDao.createThreadFromGroup(threadid);
            }
            bean.setThreadsId(threadid);
            threadsDao.updateLastTime(threadid, curtime);
        }
    ...
    public String createRecMeetingBookMsgBody(String extinfo) {
        ...
        if (!TextUtils.isEmpty(extinfo)) {
            try {
                ...
                if (!TextUtils.isEmpty(meeting)) {
                    JSONObject meetingObject = new JSONObject(meeting);
                    object.put(BookMeetingExInfo.BOOK_NUBE, meetingObject.optString(BookMeetingExInfo.BOOK_NUBE));
                    object.put(BookMeetingExInfo.BOOK_NAME, meetingObject.optString(BookMeetingExInfo.BOOK_NAME));
                    object.put(BookMeetingExInfo.MEETING_ROOM, meetingObject.optString(BookMeetingExInfo.MEETING_ROOM));
                    object.put(BookMeetingExInfo.MEETING_URL, meetingObject.optString(BookMeetingExInfo.MEETING_URL));
                    object.put(BookMeetingExInfo.MEETING_TIME, meetingObject.optLong(BookMeetingExInfo.MEETING_TIME));
                    object.put(BookMeetingExInfo.MEETING_THEME, meetingObject.optString(BookMeetingExInfo.MEETING_THEME));
                } else {
     ...
     *
     * @param msgid
     *            消息的msgid （尽量使用服务端返回的， 仅在收到91以前版本时，分享的多个图片拆解时，可以为空，需手动创建一个）
     * @param sender
     *            消息的发送者ID
     * @param receiver
     *            消息的接收这ID (一般为自己登录的帐号ID)
     * @param body
     *            消息体，已经使用上面的三个方法转化为本地的Json数组；
     *            createRecPAVMsgBody()---图片、声音、视频类型转换；
     *            createRecCardMsgBody()---名片转换 ；
     *            createRecTxtMsgBody---文字转换；
     *            createRecRecordMsgBody()---通话记录/会议邀请记录 转换
     * @param type
     *            消息的类型（图片、声音、视频、名片、文字、通话/会议记录）
     * @param title
     *            消息的标题
     * @param extinfo
     *            消息的扩展字段
     ...
     *            消息的发送时间
     * @param groupid
     *            如果是群组聊天信息需要将group id作为会话的thread id
    ...
    public String createReceiveMsgNotice(String msgid, String sender,
       ...
       CustomLog.d(TAG,"createReceiveMsgNotice msgid:" + msgid + "|sender:" + sender
        ...
        if(TextUtils.isEmpty(groupid)){
            // 普通的点对点消息
            // 接收方一般是单个用户，认为是已经有序的
            String covstid = threadsDao.createThread(sender, curtime, true);
            if (TextUtils.isEmpty(covstid)) {
               CustomLog.d(TAG,"createRecieverFileNotice createThread id==null");
                return "";
            }
            bean.setThreadsId(covstid);
        }else{
            // 群组聊天消息
            // 需要判断下，当前是否已建立了群组的会话，应当提前建立；
            if (!threadsDao.isExistThread(groupid)) {
                threadsDao.createThreadFromGroup(groupid);
            }
            bean.setThreadsId(groupid);
            threadsDao.updateLastTime(groupid, curtime);
        }
        // 下面两个字段不再维护
        bean.setMsgId("");
        bean.setFailReplyId("");
    ...
    public String createAddFriendTxt(String msgid,String receiver,
                                     List<String> localfiles, String title, int type, String txt,
       ...
       CustomLog.d(TAG,"after sorted recipentIds:" + recipentIds);
        ...
        if (TextUtils.isEmpty(threadid)) {
            String covstid = threadsDao
                    .createThread(recipentIds, serTime, true);
            if (TextUtils.isEmpty(covstid)) {
               CustomLog.d(TAG,"createSendFileNotice createThread id==null");
                return "";
            }
            ...
            bean.setThreadsId(threadid);
            if(serTime == 1){
                threadsDao.updateLastTime(threadid, System.currentTimeMillis());
            }else{
                threadsDao.updateLastTime(threadid, serTime);
            }
        }
        // 下面两个字段不再维护
        bean.setMsgId("");
