...
public class CollectionManager{
    ...
    public void addCollectionByNoticesBean(Context context,NoticesBean bean){
        ...
        if(loginUserInfo.getNube().equals(bean.getSender())){
            newBody = addNewMsgToBody(bean.getBody(),loginUserInfo.getNickName(),loginUserInfo.getHeadThumUrl(),loginUserInfo.getNube());
        }else{
            ContactFriendBean friendBean = new NetPhoneDaoImpl(context).queryFriendInfoByNube(bean.getSender());
            if (friendBean != null && !TextUtils.isEmpty(friendBean.getNubeNumber())){
                newBody = addNewMsgToBody(bean.getBody(),friendBean.getNickname(),friendBean.getHeadUrl(),friendBean.getNubeNumber());
            }else{
                newBody = addNewMsgToBody(bean.getBody(),bean.getSender(),"",bean.getSender());
            }
        }
    ...
    private String addNewMsgToBody(String oldBody,String name,String headUrl,String operatorNube){
    ...
    public void addCollectionFromPic(Context context,List<String> localfiles){
        ...
        entity.setOperatorNube(MedicalApplication.getPreference().getKeyValue(PrefType.LOGIN_NUBENUMBER, ""));
    ...
    public void deleteCollectionById(String collectId) {
        if(collectId.length() == 0){
            CustomLog.d("CollectManage","deleteCollectionById collectId 为空 collectId:" + collectId);
            return;
        }
    ...
    public boolean showForwordAndDelete(final Activity activity,final CollectionBean bean){
        ...
        if (FileTaskManager.NOTICE_TYPE_TXT_SEND==bean.getType()||
                ...
                FileTaskManager.NOTICE_TYPE_PHOTO_SEND==bean.getType()||FileTaskManager.NOTICE_TYPE_FILE==bean.getType()){
...
//	}
    ...
    private void playAudio(String audioPath, Context context) {
        ...
        try {
            ((AudioManager) context.getApplicationContext().getSystemService(Context.AUDIO_SERVICE)).setMode(AudioManager.MODE_NORMAL);
            mMediaPlayer = MediaPlayer.create(context.getApplicationContext(),Uri.fromFile(new File(audioPath)));
