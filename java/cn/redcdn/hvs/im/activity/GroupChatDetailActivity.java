...
public class GroupChatDetailActivity extends BaseActivity implements OnClickListener
    ...
    private TextView groupAnnouncement, groupDefaultNotice;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case QUIT_BY_SELF:
                    ...
                    showLoadingView("正在退出群聊", new DialogInterface.OnCancelListener() {
                        @Override
                ...
                case SAVE_TO_CONTACT:
                    ...
                    if (isSaveToContact) {
                        ...
                        GroupChatDetailActivity.this.showLoadingView("保留群组中...");
                        ...
                        GroupChatDetailActivity.this.showLoadingView("取消保留...");
                ...
                case QUERY_GROUP_DETAIL:
                    ...
                    showLoadingView("正在查询群详情", new DialogInterface.OnCancelListener() {
                        @Override
                        public void onCancel(DialogInterface dialogInterface) {
                ...
                , new ContactCallback() {
                    ...
                    public void onFinished(ResponseEntry result) {
                        ...
                        if (result.status >= 0) {
                            ...
                            CustomToast.show(GroupChatDetailActivity.this, "添加成功！", 1);
                        } else {
                            CustomLog.i(TAG, "onFinish! status: " + result.status
                                + " | content: " + result.content);
                            CustomToast.show(GroupChatDetailActivity.this, "添加失败！", 1);
                ...
                new ContactCallback() {
                    ...
                    public void onFinished(ResponseEntry result) {
                        ...
                        if (result.status >= 0) {
                            ...
                            if (!isExitGroup) {
                                CustomToast.show(GroupChatDetailActivity.this, "取消保存通讯录成功！", 1);
                            }
                        } else {
                            CustomLog.d(TAG, "取消保存通讯录失败");
                            if (!isExitGroup) {
                                CustomToast.show(GroupChatDetailActivity.this, "取消保存通讯录失败！", 1);
                            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        switch (requestCode) {
            ...
            case ACTIVITY_GROUP_ANNOUNCEMENT:
                CustomToast.show(GroupChatDetailActivity.this, "发送群公告", CustomToast.LENGTH_SHORT);
...
//        final CommonDialog mDialog = new CommonDialog(this, getLocalClassName(), 414);
        ...
        switch (type) {
            case CLEAR_MESSAGE:
                ...
                title = "清空聊天记录";
                mDialog.setTip("清空聊天记录吗？");
                break;
            case QUIT_BY_SELF:
                CustomLog.d(TAG, "弹出退出群聊对话框");
                title = "退出群聊";
                mDialog.setTip("退出后将不再接收群的聊天消息，同时清除该群的聊天记录，确定退出？");
                break;
            case REMOVED_LOADER_TO_OTHER:
                title = "退出群聊";
                mDialog.setTip("你将群主移交给" + dateList.get(nextLoaderNube).getDispName() +
                    "，退出后将不再接收群的聊天消息，同时清除该群的聊天记录，确定退出？");
...
//                GroupChatDetailActivity.this.finish();
    ...
    private void updateView() {
        ...
        if (mChatType.equals(VALUE_GROUP)) {
            ...
            titleBar.setTitle("群聊信息" + "(" + dateList.size() + ")");
            ...
            if (mGroupBean != null) {
                ...
                showGroupName.setText(
                    TextUtils.isEmpty(mGroupBean.getgName()) ? "未命名" : mGroupBean.getgName());//群名称
            } else {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.clear_group_chat_message_relativelayout://清空聊天记录
                if (!Conversation) {
                    CustomToast.show(GroupChatDetailActivity.this, "尚无会话信息，不可点击", 1);
                    return;
                }
            ...
            case R.id.group_accouncement_layout:
                ...
                if (itemBean == null){
                    CustomToast.show(GroupChatDetailActivity.this,"群主已退群，无法编辑群公告",1);
                }else {
...
//                CustomLog.d(TAG, "点击直接退出");
    ...
    private void getAnnouncement() {
        ...
        String loginUserToken = AccountManager.getInstance(this).getToken();
    ...
    private void sendGroupMsg(String strAnnouncement) {
        ...
        final String sendMsg = "@所有人:" + strAnnouncement;
