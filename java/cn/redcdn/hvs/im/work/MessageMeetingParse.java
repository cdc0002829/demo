...
public class MessageMeetingParse extends MessageBaseParse {

    private PrivateMessage msg = null;
    private int type = -1;
    private ExtInfo extInfo = null;

    public MessageMeetingParse(PrivateMessage msg){
        this.msg = msg;
        this.type = FileTaskManager.NOTICE_TYPE_MEETING_INVITE;
    }
    ...
    public boolean parseMessage(){
        ...
        switch(verInt){
            case BizConstant.MSG_VERSION_INT_BASE_X:
                CustomLog.d("MessageMeetingParse","91以前消息版本,不解析直接丢弃");
                break;
            case BizConstant.MSG_VERSION_INT_BASE_00:
            default:{
                //把多张图片拆解成多条记录插入到本地
                IMCommonUtil.setKeyValue(txt, msg.gid);
                String body = noticesDao.createRecMeetingInviteMsgBody(msg.extendedInfo);
                ...
                CustomLog.d("MessageMeetingParse","V1.00消息版本，插入本地文字 uuid="+uuid);
